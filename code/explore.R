...
#madata %>% group_by(file.nr) %>% distinct(study.name) %>% count() %>% filter(n < 2) -> File.nr 38 for example
arrange(filter(madata, file.nr == 38) %>% 
...
null.events.single <- madata %>% filter(is.na(mean1) | mean1 == 0) %>% filter(is.na(mean2) | mean2 == 0) %>% 
  filter(is.na(sd1) | sd1 <= 0) %>% filter(is.na(sd2) | sd2 <= 0) %>% 
  summarise(null.ev = length(total1[events1 <= 0 | events2 <= 0]), total = length(events1)) %>% 
  mutate(f = null.ev/total) %>% select(f)
