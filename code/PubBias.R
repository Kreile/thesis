...
meta.bin.complete <- function(data, min.study.number, sig.level){
  metadat <- data %>% filter(outcome.measure == "Risk Ratio" | outcome.measure == "Odds Ratio") %>% filter(file.nr != 3014) %>% 
    ...
    summarize(doi = unique(doi), 
              ...
              pval.copas.bin = 2*(1 - pnorm(abs(lor.copas.bin/se.lor.copas.bin))),
              sig.copas.bin = ifelse(pval.trimfill.bin > sig.level, 0, 1),
              sig.change.ranef.reg = sig.change(sig.ranef.bin, sig.after =  sig.reg.ranef.bin),
              ...
              stat.rucker.bin = metabias(metabin(event.e = events1, n.e = total1, event.c = events2, n.c = total2, studlab = study.name, sm = "ASD"), 
                                         method = "mm")$statistic,
              missing.trim.bin = trimfill(metabin(event.e = events1, n.e = total1, event.c = events2, n.c = total2, studlab = study.name))$k0 / n(),
              Q.bin = metabin(event.e = events1, n.e = total1, event.c = events2, n.c = total2, studlab = study.name, sm = "OR")$Q,
              pval.Q.bin = metabin(event.e = events1, n.e = total1, event.c = events2, n.c = total2, studlab = study.name, sm = "OR")$pval.Q,
              sig.Q.bin = ifelse(pval.Q.bin < sig.level, 1, 0),
              I2  = max(0, (Q.bin - n + 1)/Q.bin)) 
  return(metadat)
} 
...
meta.cont.complete <- function(data, min.study.number, sig.level){
  metadat <- data %>% filter(outcome.measure == "Mean Difference" | outcome.measure == "Std. Mean Difference") %>%# filter(file.nr < 503) %>% 
    ...
    summarize(doi = unique(doi), 
              ...
              sig.change.ranef.reg = sig.change(sig.ranef.cont, sig.after =  sig.reg.ranef.cont),
              sig.change.ranef.copas = sig.change(sig.ranef.cont, sig.after =  sig.copas.cont),
...
save(meta.cont, file = "PubBias_results4.RData")
...
#Dependence of significance of tests on mean and total samplesize
...
ds <- data %>% filter(file.nr == 11) %>% group_by(file.nr, comparison.nr, outcome.nr, subgroup.nr) %>% 
  filter(outcome.measure == "Odds Ratio" | outcome.measure == "Risk Ratio") %>% filter(events1 > 0 & events2 > 0) %>% 
  mutate(counts = n()) 
print(ds %>% select(effect, events1, events2, study.name, outcome.name, comparison.name, counts), n = 200)

meta.ds <- metabin(event.e = events1, n.e = total1, event.c = events2, n.c = total2, data = ds, studlab = study.name, sm = "OR")
summary(meta.ds)
limit.ds <- limitmeta(meta.ds)
summary(limit.ds)


ds <- data %>% filter(file.nr == 8, outcome.nr == 1, subgroup.nr == 1) %>% group_by(file.nr, outcome.nr, comparison.nr, subgroup.nr) %>% 
  filter(outcome.measure == "Odds Ratio" | outcome.measure == "Risk Ratio") %>% filter(events1 > 0 & events2 > 0) %>% 
  mutate(counts = n()) %>% filter(counts == 18)
print(ds %>% select(effect, events1, events2, study.name, comparison.name, outcome.name, counts), n = 200)
...
if(any(pval.rsb < sig.level)){
  ...
  if(any(pval.rsb < sig.level)){
    ...
    ind.estimate <- which.min(pval.rsb)
    corr.est <- estimates[ind.estimate]
  }
} else{
  if(any(pval.rsb > sig.level)){
    corr.est <- NA
...
meta.cont %>% ggplot(aes(x = pval.thomson.cont, y = trim.cont)) + geom_point(alpha = 0.5) + theme_bw() #+ geom_smooth(method = "loess") 
...
# meta.cont.plot <- meta.cont %>% ungroup() %>% select(pval.egger.cont, pval.thomson.cont, pval.begg.cont, trim.cont, Q, I2)
