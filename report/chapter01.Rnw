...
require(tidyverse)
par(mfrow = c(1,2))
tp <- data.ext2 %>% filter(meta.id == pb.ex)
funnel(metabin(event.e = events1, event.c = events2, n.e = total1, n.c = total2, data = tp),
       comb.fixed = F, comb.random = F)

nopb.ex <- meta.f$meta.id[32]
tp <- data.ext2 %>% filter(meta.id == nopb.ex)
funnel(metabin(event.e = events1, event.c = events2, n.e = total1, n.c = total2, data = tp),
       comb.fixed = F, comb.random = F)
@
\end{figure}
