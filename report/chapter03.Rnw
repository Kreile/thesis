...
meta <- meta %>% mutate(sig.ranef = ifelse(!is.na(sig.ranef.bin), sig.ranef.bin, sig.ranef.cont),
                        ...
                        sig.reg.random = ifelse(!is.na(sig.reg.random.bin), sig.reg.random.bin, sig.reg.random.cont),
                        missing.trim = ifelse(!is.na(missing.trim.bin), missing.trim.bin, missing.trim.cont),
                        pval.thomson = ifelse(!is.na(pval.rucker.bin), pval.rucker.bin, pval.thomson.cont))
@
...
A somewhat different question is how statistical power of studies changed over time. As an approximation, the total sample size of a study can be used. Figure \ref{median.samplesize.overtime} shows the median sample size of all studies of a given year. Again, to reduce the effects of distortion due to random fluctuation, only the medians for the years 1980 to 2017 are used ($n$ > 50,000).
...
# 
# meta.cont %>% ggplot(aes(x = missing.missing.trim.cont)) + geom_histogram(col = "gray15", fill = "dodgerblue") +
#   theme_bw() + labs(title = "Fraction of Trimmed Comparisons (continuous outcome)") + xlab("Fraction") + ylab("Frequency")
# 
# meta.bin %>% ggplot(aes(x = missing.trim.bin, fill = factor(peter.test))) + geom_density(na.rm = T, position = "fill")
# 
# meta.bin %>% ggplot(aes(x = missing.trim.bin, fill = factor(harbord.test))) + geom_density(na.rm = T, position = "fill")
# 
# meta.cont %>% ggplot(aes(x = missing.missing.trim.cont, fill = factor(thomson.test))) + geom_density(na.rm = T, position = "fill")
# 
