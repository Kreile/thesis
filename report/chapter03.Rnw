...
dt <- meta.iv %>% group_by(outcome.measure.new) %>% count
...
The dataset that was ultimately tested and adjusted for publication bias comprises \Sexpr{length(meta.id.vector)} meta-analyses and \Sexpr{dim(meta.data)[1]} results. The mean number of participants in the treatment group is \Sexpr{mean(meta.data$total1, na.rm = T)} vs \Sexpr{mean(data.ext2$total1, na.rm = T)} in the unrestricted dataset and the mean total number of participants \Sexpr{mean(meta.data$total1 + meta.data$total2)} vs \Sexpr{mean(data.ext2$total1 + data.ext2$total2)}. The mean publication year is \Sexpr{mean(meta.data$study.year, na.rm = TRUE)} vs \Sexpr{mean(data.ext2$study.year, na.rm = TRUE)} in the unrestricted dataset. \\
From the meta-analyses with incomplete data (\texttt{outcome.type} == \texttt{IV}}), there are \Sexpr{first.iv.outcome.count} with \texttt{outcome.measure.new} = ..., \Sexpr{second.iv.outcome.count} ...,  \Sexpr{third.iv.outcome.count} ... and \Sexpr{fourth.iv.outcome.count} ... .
