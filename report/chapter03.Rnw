...
It is possible to analyse all studies with one or more replica by meta-analysis. There are different meta-analyses that can be applied. First, it is investigated how well the significance of a effect size estimate of a study replicates in significance of the pooled effect size estimate of a meta-analysis. First, the amount of significant study effect estimates is shown over time. The fraction of significant effect size estimates on the $p$-value threshold of $0.05$ is only shown for times where a reasonbly large number of effect estimates is available in order to reduce random fluctuation ($n > 800$). Shown are p-values for significance tests where the outcome measure of the study was either a risk ratio, odds ratio, mean difference and standardized mean difference. Additionally, only study years between 1970 and 2017 have been included and the sample size for each arm of the study had to $> 12$.
...
% \end{figure}
% 
% A different question is how statistical power of studies changed over time. As an approximation, the total sample size of a study can be used. Figure \ref{median.samplesize.overtime} shows the median sample size of all studies of a given year. Again, to reduce the effects of distortion due to random fluctuation, only the medians for the years 1980 to 2017 are used ($n$ > 50,000).
% 
% \begin{figure}
% <<echo=FALSE>>=
...
% @
% \caption{Median sample size over time (years).}
% \label{median.samplesize.overtime}
% \end{figure}


\subsection{Meta-analyses}

Meta-analyses can be applied on the dataset given that there are studies that have one or more replica (i.e. same comparison type, outcome type and subgroup type). 
...
p.change.ranef.bin <- meta.bin %>% ungroup() %>% 
  ...
  annotate("text", x = change.ranef.bin[change.ranef.bin$sig.change == 3,]$correction.method, y = 300, 
...
p.change.ranef.cont <- meta.cont %>% ungroup() %>% 
  ...
  theme_bw() + xlab(label = NULL) + scale_fill_discrete(labels= c("unchanged.nonsig", "change.to.sig", "unchanged.sig", "change.to.nonsig")) +
  annotate("text", x = change.ranef.cont[change.ranef.cont$sig.change == 3,]$correction.method, y = 200, 
...
p.ranef.change <- meta %>% ungroup() %>% 
  ...
  theme_bw() + xlab(label = NULL) + scale_fill_discrete(labels= c("unchanged.nonsig", "change.to.sig", "unchanged.sig", "change.to.nonsig")) +
  annotate("text", x = change.ranef[change.ranef$sig.change == 3,]$correction.method, y = 500, 
...
p.change.fixef.bin <- meta.bin %>% ungroup() %>% 
  ...
  ggplot(aes(x = correction.method, fill = sig.change)) + geom_bar() + coord_flip() + ggtitle("Binary outcomes")+  
  theme_bw() + xlab(label = NULL) + scale_fill_discrete(labels= c("unchanged.nonsig", "change.to.sig", "unchanged.sig", "change.to.nonsig")) +
  annotate("text", x = change.fixef.bin[change.fixef.bin$sig.change == 3,]$correction.method, y = 200, 
...
p.fixef.change <- meta %>% ungroup() %>% 
  ...
  ggplot(aes(x = correction.method, fill = sig.change)) + geom_bar() + coord_flip() + 
  theme_bw() + xlab(label = NULL) + scale_fill_discrete(labels= c("unchanged.nonsig", "change.to.nonsig", "unchanged.sig", "change.to.sig")) +
  annotate("text", x = change.fixef[change.fixef$sig.change == 3,]$correction.method, y = 500, 
...
\subsubsection{Small Study Effect Tests}
...
\begin{figure}
<<echo=FALSE>>=
grid.arrange(p.fixef.change.bin, p.fixef.change.cont, ncol = 2)
@
...
\begin{figure}
<<echo=FALSE>>=
grid.arrange(p.ranef.change.bin, p.ranef.change.cont, ncol = 2)
@
