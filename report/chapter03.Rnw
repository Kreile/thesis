...
tp <- data.ext2 %>% filter(id %in% data.review$id[which(data.review$withdrawn == FALSE)]) %>% 
  filter(outcome.flag == "DICH" | outcome.flag == "CONT" | outcome.flag == "IV")
...
m.a.variance4.count <- data.ext2 %>% filter(meta.id %in% m.a.largerten.id$meta.id) %>% 
  group_by(meta.id) %>% summarise(ratio = (max(se.new)^2)/(min(se.new)^2)) %>% 
  filter(ratio >= 4) %>% count
tp3 <- data.ext2 %>% filter(meta.id %in% m.a.largerten.id$meta.id) %>% 
  group_by(meta.id) %>% mutate(ratio = (max(se.new)^2)/(min(se.new)^2)) %>% 
  filter(ratio >= 4)
...
#smaller I2 than 0.5:
m.a.smaller.5.count <- meta.info.I2 %>% filter(I2 < 0.5) %>% ungroup %>% count()
...
#Adjustment dataset:
meta.adj <- meta.f %>% filter(outcome.flag == "DICH" | outcome.flag == "CONT")
...
The dataset consists of \Sexpr{length(unique(data$id))} systematic reviews from the Cochrane Library with \Sexpr{length(unique(data$study.name))} studies and \Sexpr{dim(data)[1]} results. The results all stem from comparisons of a healthcare interventions or treatments to a reference. The reference may be a placebo control group or a other intervention or treatment. A result can not only be about efficacy of the treatments, but also about safety (adverse effects).\\
...
print(xtable(cum.repr.trials.subg, label = "repr.groups", caption = "Number and cumulative number of groups with meta-analysis size n.", align = "llrr", digits = 0), include.rownames = F, size = "footnotesize")
@
...
% If not computable from \texttt{mean1}, \texttt{mean2}, \texttt{sd1} and \texttt{sd2}, and \texttt{outcome.measure.merged} was equal to ``Std. Mean Difference'', it was set equal to \texttt{effect} (and \texttt{var.smd} equal to \texttt{se} squared, $n$ =  \Sexpr{length(inpute.given.ind)}).
...
Initially, the analysis is restricted to results with \texttt{outcome.flag} \texttt{DICH}, \texttt{CONT} and \texttt{IV}. binary, continuous and survival outcomes. This corresponds to \Sexpr{length(data.ext2$outcome.flag[which(data.ext2$outcome.flag == "DICH" | data.ext2$outcome.flag == "CONT" | data.ext2$outcome.flag == "IV")])} out of a total of \Sexpr{length(data.ext2$outcome.flag)} results.
\citet{Ioannidis2007} outlined criteria for application of small study effect tests:
\begin{itemize}
\item \textbf{Sample size}: A meta-analysis is comprised of at least ten studies ($n$ = \Sexpr{m.a.largerten.count} remaining). 
\item \textbf{Study size}: The ratio between largest variance of an estimate and smallest variance of an estimate is larger than four ($n$ = \Sexpr{m.a.variance4.count} remaining).
\item \textbf{Significance}: At least one treatment effect has a $p$-value below the significance threshold 0.05 ($n$ = \Sexpr{m.a.one.sig.count} remaining)
\item \textbf{Heterogeneity}: The $I^2$ statistic of a given meta-analysis is smaller than 0.5, thus, the proportion of between study variance of the overall variance is smaller than 0.5 ($n$ = \Sexpr{m.a.smaller.5.count} remaining).
\end{itemize}
...
The dataset that was ultimately tested and adjusted for publication bias comprises \Sexpr{length(meta.id.vector)} meta-analyses and \Sexpr{dim(meta.data)[1]} results. The mean number of participants in the treatment group is \Sexpr{round(mean(meta.data$total1, na.rm = T),1)} vs \Sexpr{round(mean(data.ext2$total1, na.rm = T),1)} in the unrestricted dataset and the mean total number of participants \Sexpr{round(mean(meta.data$total1 + meta.data$total2),1)} vs \Sexpr{round(mean(data.ext2$total1 + data.ext2$total2),1)}. The mean publication year is \Sexpr{mean(meta.data$study.year, na.rm = TRUE)} vs \Sexpr{mean(data.ext2$study.year, na.rm = TRUE)} in the unrestricted dataset. \\
From the meta-analyses with incomplete data (\texttt{outcome.type} == \texttt{IV}), there are \Sexpr{first.iv.outcome.count} with \\ \texttt{outcome.measure.merged} = ``Risk Ratio'', \Sexpr{second.iv.outcome.count} ``Std. Mean Difference'',  \Sexpr{third.iv.outcome.count} ``Hazard Ratio'' and \Sexpr{fourth.iv.outcome.count} ``Odds Ratio'' (and 25 additional, different outcome measures).
