...
tp <- data %>% filter(outcome.flag == "DICH" | outcome.flag == "CONT" | outcome.flag == "IV")
...
m.a.largerten.bin <- data.ext2 %>% filter(outcome.flag == "DICH") %>% 
...
m.a.largerten.cont <- data.ext2 %>% filter(outcome.flag == "CONT") %>% 
  filter(sd1 != 0 & sd2 != 0) %>% 
  filter(total1 > 0 & total2 > 0) %>% group_by(meta.id) %>% 
  filter(n() > 9) %>% distinct(meta.id) %>% ungroup() %>% mutate(n = n())
  

m.a.largerten.iv <- data.ext2 %>% filter(outcome.flag == "IV") %>% 
...
dt <- meta.iv %>% group_by(outcome.measure.new) %>% count %>% arrange(desc(n))
...
% If not computable from \texttt{mean1}, \texttt{mean2}, \texttt{sd1} and \texttt{sd2}, and \texttt{outcome.measure.new} was equal to ``Std. Mean Difference'', it was set equal to \texttt{effect} (and \texttt{var.smd} equal to \texttt{se} squared, $n$ =  \Sexpr{length(inpute.given.ind)}).
...
From the meta-analyses with incomplete data (\texttt{outcome.type} == \texttt{IV}), there are \Sexpr{first.iv.outcome.count} with \\ \texttt{outcome.measure.new} = ``Risk Ratio'', \Sexpr{second.iv.outcome.count} ``Std. Mean Difference'',  \Sexpr{third.iv.outcome.count} ``Hazard Ratio'' and \Sexpr{fourth.iv.outcome.count} ``Odds Ratio'' (and 25 additional, different outcome measures).
