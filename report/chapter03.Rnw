...
p.dist.bin <- meta.bin %>% ungroup() %>% 
	...
	geom_text(data = dat_text, mapping = aes(x = 0.5, y = 300, label = label), color = "black") + 
...
p.dist.cont <- meta.cont %>% ungroup() %>% 
	...
	geom_text(data = dat_text, mapping = aes(x = 0.5, y = 90, label = label),  color = "black") + 
	theme(strip.text.x = element_text(size=10))
#--------------------------------------------------------------------------------------------------------------------#
...
p.dist.surv <- meta.surv %>% ungroup() %>% 
	...
	ggplot(aes(x = p.value)) + geom_histogram(boundary = 0, binwidth = 0.1) + theme_bw() + 
	facet_wrap(~test.type, labeller = labeller(test.type = labels)) + 
	geom_text(data = dat_text, mapping = aes(x = 0.5, y = 11, label = label),  color = "black") + 
	theme(strip.text.x = element_text(size=10))
#--------------------------------------------------------------------------------------------------------------------#
...
When adjusting for small study effects, the newly obtained test statistics, as well as the corrected effect size is of interest. The former is displayed in Figures \ref{fig:adjustment.z} for the adjusted $z$-scores, \ref{fig:adjustment.smd} for the adjusted Hedges $g$ and Cohen's $d$ and \ref{fig:adjustment.log.hazard.ratio} for the adjusted log hazard ratios.
