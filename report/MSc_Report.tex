...
To compare significant findings for small study effect tests and excess significance tests, we limit ourselves to comparison with Harbord's or Egger's test. 22.4\% of binary outcome analyses had at least one of the two test $p$-values being significant, and equivalently, 23.4\% for continuous and survival outcomes. The numbers change to 13.4\% for binary outcomes and 13.4\% for continuous and survival outcomes after applying the Bonferroni correction. \\
2.7\% have significant Harbord's test result and significant excess significance test result  (1.1\% with Bonferroni). Of the continuous outcomes, we have 3.3\% with Egger's test and 0.8\% with Bonferroni correction. 
...
\begin{tabular}{lcccc}
  ...
  \hline
\hline
Excess significance, Schwarzer & 0.82 & 0.87 & 0.14 & 0.19 \\ 
  Excess significance, Peter & 0.80 & 0.84 & 0.27 & 0.21 \\ 
  Excess significance, Rucker & 0.83 & 0.85 & 0.32 & 0.26 \\ 
  Excess significance, Harbord & 0.80 & 0.85 & 0.27 & 0.24 \\ 
  ...
  Harbord, Peter & 0.86 & 0.90 & 0.53 & 0.48 \\ 
   \hline
Excess significance, Egger & 0.79 & 0.81 & 0.72 & 0.61 \\ 
  Excess significance, Thompson & 0.82 & 0.83 & 0.72 & 0.61 \\ 
  Excess significance, Begg & 0.81 & 0.84 & 0.33 & 0.28 \\ 
  Thompson, Egger & 0.95 & 0.96 & 0.96 & 0.95 \\ 
  Thompson, Begg & 0.87 & 0.89 & 0.69 & 0.73 \\ 
  Egger, Begg & 0.86 & 0.87 & 0.78 & 0.77 \\ 
   \hline
Excess significance, Egger (survival) & 0.87 & 0.97 & 0.00 &  \\ 
  Excess significance, Thompson (survival) & 0.92 & 0.97 & 0.00 &  \\ 
  Excess significance, Begg (survival) & 0.95 & 1.00 & 0.00 &  \\ 
...
Since we defined $w_i = 1/(\se_i^2 + \tau^2)$, it als holds that
...
Let $\theta/s$ be the dependent variable $y$ and $1/s$ the explanatory variable $x$. If plotted, this corresponds to a radial or Galbraith plot \citep{galbraith}. The linear regression equation as introduced before in \ref{eq:simple.regression} can be written in two ways:
...
It uses a different treatment effect and variance estimate: the score $\varphi$ of the log-likelihood, evaluated at log odds ratio $\theta_0 = 0$ and its inverse Fisher information $\se^2$. Formally,

\begin{align}
\varphi &= e_t - (e_t - e_c)(e_t + (n_t - e_t))/(n_t + n_c) \label{harbord.score} \\
 \se^2&= \frac{(e_t + e_c)(e_t + (n_t - e_t))(e_c + (n_c - e_c))((n_t - e_t) + (n_c - e_c))}{(n_t + n_c)^2(n_t + n_c - 1)} \label{harbord.variance}
\end{align}
...
\begin{align}
p &= \sum_{i = O}^n(n \choose i p^i (1-p)^{n - i})
\end{align}
