...
% \item \texttt{outcome.flag}: A simplification of \texttt{outcome.measure.merged}. The outcome.flag ``bin'' indicates if \texttt{outcome.measure.merged} is either one of ``Risk Ratio'', ''Odds Ratio'', ``Risk difference'' or ``Peto Odds Ratio''. ``cont'' is equivalent to \texttt{outcome.measure.merged} equal to "Std. Mean Difference'' or ``Mean Difference''. ``surv'' equal to ``Hazard Ratio'' and ``rate'' equal to ''Rate Ratio'' or ``Rate Difference''.
...
% If not computable from \texttt{mean1}, \texttt{mean2}, \texttt{sd1} and \texttt{sd2}, and \texttt{outcome.measure.merged} was equal to ``Std. Mean Difference'', it was set equal to \texttt{effect} (and \texttt{var.smd} equal to \texttt{se} squared, $n$ =  length(inpute.given.ind)).
...
\definecolor{shadecolor}{rgb}{0.98, 0.98, 0.98}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Dataset processing function to get transformed effect sizes, p-values, event }
\hlcom{# counts with increments, etc. :}
\hlstd{pb.process3} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{data}\hlstd{)\{}
  \hlstd{data} \hlkwb{<-} \hlstd{data} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}\hlkwc{meta.id} \hlstd{=}
            \hlkwd{group_indices}\hlstd{(., id, comparison.id, outcome.id, subgroup.id))} \hlopt{%>%}
    \hlkwd{group_by}\hlstd{(meta.id)} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}\hlkwc{study.id2} \hlstd{=} \hlkwd{row_number}\hlstd{())} \hlopt{%>%} \hlkwd{ungroup}\hlstd{()}

  \hlcom{#Mark duplicate effects between meta-analyses:}
  \hlstd{data} \hlkwb{<-} \hlstd{data} \hlopt{%>%} \hlkwd{group_by}\hlstd{(meta.id)} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{n} \hlstd{=} \hlkwd{n}\hlstd{())}  \hlopt{%>%} \hlkwd{ungroup}\hlstd{()} \hlopt{%>%} \hlkwd{group_by}\hlstd{(id)} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{dupl.id} \hlstd{=}
         \hlkwd{dupl.finder}\hlstd{(}\hlkwc{effects} \hlstd{= effect,} \hlkwc{names} \hlstd{= study.name,} \hlkwc{metas} \hlstd{= meta.id),}
           \hlkwc{dupl.remove} \hlstd{=}
  \hlkwd{dupl.max.finder}\hlstd{(}\hlkwc{duplicate.index} \hlstd{= dupl.id,} \hlkwc{study.number} \hlstd{= n,} \hlkwc{metas} \hlstd{= meta.id))} \hlopt{%>%}
    \hlkwd{ungroup}\hlstd{()}

  \hlstd{data} \hlkwb{<-} \hlstd{data} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}\hlkwc{lrr} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{var.lrr} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{hedgesg} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{var.hedgesg} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{cohensd} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{var.cohensd} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{smd.ordl} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{var.smd.ordl} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{pval.single} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{cor.pearson} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{var.cor.pearson} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{z} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{var.z} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{events1c} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{events2c} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{sig.single} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{smd.pool} \hlstd{=} \hlnum{NA}\hlstd{,}
    \hlkwc{se.smd.pool} \hlstd{=} \hlnum{NA}\hlstd{)}


  \hlstd{cont.ind} \hlkwb{<-} \hlkwd{which}\hlstd{(data}\hlopt{$}\hlstd{outcome.flag} \hlopt{==} \hlstr{"CONT"}\hlstd{)}
  \hlstd{bin.ind} \hlkwb{<-} \hlkwd{which}\hlstd{(data}\hlopt{$}\hlstd{outcome.flag} \hlopt{==} \hlstr{"DICH"}\hlstd{)}
  \hlstd{IV.ind} \hlkwb{<-} \hlkwd{which}\hlstd{(data}\hlopt{$}\hlstd{outcome.flag} \hlopt{==} \hlstr{"IV"}\hlstd{)}
  \hlcom{#----------------------------------------------------------------------------------------------#}

  \hlcom{#Outcome.flag == "DICH"}
  \hlstd{data[bin.ind,]} \hlkwb{<-} \hlkwd{escalc}\hlstd{(}\hlkwc{data} \hlstd{= data[bin.ind,],} \hlkwc{ai} \hlstd{= events1,} \hlkwc{n2i} \hlstd{= total2,}
                           \hlkwc{ci} \hlstd{= events2,} \hlkwc{n1i} \hlstd{= total1,}
                           \hlkwc{to} \hlstd{=} \hlstr{"only0"}\hlstd{,}
                           \hlkwc{add} \hlstd{=} \hlnum{1}\hlopt{/}\hlnum{2}\hlstd{,}
                           \hlkwc{measure} \hlstd{=} \hlstr{"RR"}\hlstd{,} \hlkwc{append} \hlstd{= T,}
                      \hlkwc{var.names} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"lrr"}\hlstd{,} \hlstr{"var.lrr"}\hlstd{))} \hlcom{#Calculate Risk Ratios}

  \hlstd{data[bin.ind,]} \hlkwb{<-} \hlkwd{escalc}\hlstd{(}\hlkwc{data} \hlstd{= data[bin.ind,],} \hlkwc{ai} \hlstd{= events1,} \hlkwc{n2i} \hlstd{= total2,}
                           \hlkwc{ci} \hlstd{= events2,} \hlkwc{n1i} \hlstd{= total1,}
                           \hlkwc{to} \hlstd{=} \hlstr{"only0"}\hlstd{,}
                           \hlkwc{add} \hlstd{=} \hlnum{1}\hlopt{/}\hlnum{2}\hlstd{,}
                           \hlkwc{measure} \hlstd{=} \hlstr{"OR2DL"}\hlstd{,} \hlkwc{append} \hlstd{= T,}
  \hlkwc{var.names} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"smd.ordl"}\hlstd{,} \hlstr{"var.smd.ordl"}\hlstd{))} \hlcom{#Calculate SMD (w. logistic transf.)}

  \hlcom{#What to do if there are 0/total cells:}
  \hlstd{data[bin.ind,]} \hlkwb{<-} \hlstd{data[bin.ind,]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{events1c}  \hlstd{=} \hlkwd{case_when}\hlstd{(events1} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events1} \hlopt{+} \hlnum{0.5}\hlstd{,}
                                 \hlstd{events2} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events1} \hlopt{+} \hlnum{0.5}\hlstd{,}
                                 \hlstd{events1} \hlopt{-} \hlstd{total1} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events1} \hlopt{-} \hlnum{0.5}\hlstd{,}
                                 \hlstd{events2} \hlopt{-} \hlstd{total2} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events1} \hlopt{-} \hlnum{0.5}\hlstd{,}
                                 \hlnum{TRUE} \hlopt{~} \hlstd{events1),}
           \hlkwc{events2c} \hlstd{=} \hlkwd{case_when}\hlstd{(events2} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events2} \hlopt{+} \hlnum{0.5}\hlstd{,}
                                \hlstd{events1} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events2} \hlopt{+} \hlnum{0.5}\hlstd{,}
                                \hlstd{events2} \hlopt{-} \hlstd{total2} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events2} \hlopt{-} \hlnum{0.5}\hlstd{,}
                                \hlstd{events1} \hlopt{-} \hlstd{total1} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events2} \hlopt{-} \hlnum{0.5}\hlstd{,}
                                \hlnum{TRUE} \hlopt{~} \hlstd{events2))}

  \hlcom{#What to do if there is one zero and one total:}
  \hlstd{data[bin.ind,]} \hlkwb{<-} \hlstd{data[bin.ind,]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{events1c}  \hlstd{=} \hlkwd{case_when}\hlstd{(events2} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{events1} \hlopt{-} \hlstd{total1} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events1,}
                                 \hlstd{events1} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{events2} \hlopt{-} \hlstd{total2} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events1,}
                                 \hlnum{TRUE} \hlopt{~} \hlstd{events1c),}
           \hlkwc{events2c} \hlstd{=} \hlkwd{case_when}\hlstd{(events2} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{events1} \hlopt{-} \hlstd{total1} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events2,}
                                \hlstd{events1} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{events2} \hlopt{-} \hlstd{total2} \hlopt{==} \hlnum{0} \hlopt{~} \hlstd{events2,}
                                \hlnum{TRUE} \hlopt{~} \hlstd{events2c))}
  \hlstd{data[bin.ind,} \hlstr{"pval.single"}\hlstd{]} \hlkwb{<-} \hlstd{data[bin.ind, ]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{pval.single} \hlstd{=} \hlnum{2}\hlopt{*}\hlstd{(}\hlnum{1}\hlopt{-}\hlkwd{pnorm}\hlstd{(}\hlkwd{abs}\hlstd{(lrr}\hlopt{/}\hlkwd{sqrt}\hlstd{(var.lrr)))))} \hlopt{%>%} \hlkwd{select}\hlstd{(pval.single)}
  \hlcom{#----------------------------------------------------------------------------------------------#}

  \hlcom{#Outcome.flag == "CONT"}
  \hlstd{data[cont.ind,]} \hlkwb{<-} \hlkwd{escalc}\hlstd{(}\hlkwc{data} \hlstd{= data[cont.ind,],} \hlkwc{m1i} \hlstd{= mean1,} \hlkwc{m2i} \hlstd{= mean2,}
                            \hlkwc{sd1i} \hlstd{= sd1,} \hlkwc{sd2i} \hlstd{= sd2,} \hlkwc{n1i} \hlstd{= total1,} \hlkwc{n2i} \hlstd{= total2,}
                            \hlkwc{measure} \hlstd{=} \hlstr{"SMD"} \hlstd{,} \hlkwc{append} \hlstd{= T,}
                \hlkwc{var.names} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"hedgesg"}\hlstd{,} \hlstr{"var.hedgesg"}\hlstd{))} \hlcom{#Calculate Hedge's g}

  \hlstd{data[cont.ind, ]} \hlkwb{<-} \hlstd{data[cont.ind, ]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{cohensd} \hlstd{= (mean1} \hlopt{-} \hlstd{mean2)}\hlopt{/}\hlkwd{sqrt}\hlstd{((((total1} \hlopt{-} \hlnum{1}\hlstd{)}\hlopt{*}\hlstd{sd1}\hlopt{^}\hlnum{2}\hlstd{)} \hlopt{+}
                    \hlstd{(total2} \hlopt{-} \hlnum{1}\hlstd{)}\hlopt{*}\hlstd{sd2}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{/}\hlstd{(total1} \hlopt{+} \hlstd{total2} \hlopt{-} \hlnum{2}\hlstd{)),}
           \hlkwc{var.cohensd} \hlstd{= ((total1} \hlopt{+} \hlstd{total2)}\hlopt{/}\hlstd{(total1} \hlopt{*} \hlstd{total2))} \hlopt{+}
             \hlstd{(cohensd}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{/}\hlstd{(}\hlnum{2}\hlopt{*}\hlstd{(total1} \hlopt{+} \hlstd{total2)))}

  \hlstd{data[cont.ind, ]} \hlkwb{<-} \hlstd{data[cont.ind, ]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{cohensd} \hlstd{=} \hlkwd{case_when}\hlstd{(sd1} \hlopt{==} \hlnum{0} \hlopt{~} \hlnum{NA_real_}\hlstd{,}
                               \hlstd{sd2} \hlopt{==} \hlnum{0} \hlopt{~} \hlnum{NA_real_}\hlstd{,}
                               \hlstd{total2} \hlopt{==} \hlnum{0} \hlopt{~} \hlnum{NA_real_}\hlstd{,}
                               \hlstd{total1} \hlopt{==} \hlnum{0} \hlopt{~} \hlnum{NA_real_}\hlstd{,}
                               \hlnum{TRUE} \hlopt{~} \hlstd{cohensd))}

  \hlcom{#p-value for continuous outcomes (Student):}
  \hlstd{data[cont.ind,} \hlstr{"pval.single"}\hlstd{]} \hlkwb{<-} \hlstd{data[cont.ind, ]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{t} \hlstd{= (mean1} \hlopt{-} \hlstd{mean2)}\hlopt{/}\hlstd{(}\hlkwd{sqrt}\hlstd{((((total1}\hlopt{-}\hlnum{1}\hlstd{)}\hlopt{*}\hlstd{sd1}\hlopt{^}\hlnum{2}\hlstd{)} \hlopt{+} \hlstd{(total2}\hlopt{-}\hlnum{1}\hlstd{)}\hlopt{*}\hlstd{sd2}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{/}
                          \hlstd{(total1} \hlopt{+} \hlstd{total2} \hlopt{-}\hlnum{2}\hlstd{))}\hlopt{*}\hlkwd{sqrt}\hlstd{((}\hlnum{1}\hlopt{/}\hlstd{total1)}\hlopt{+}\hlstd{(}\hlnum{1}\hlopt{/}\hlstd{total2))),}
           \hlkwc{pval.single} \hlstd{=} \hlnum{2}\hlopt{*}\hlstd{(}\hlnum{1}\hlopt{-}\hlkwd{pt}\hlstd{(}\hlkwd{abs}\hlstd{(t),} \hlkwc{df} \hlstd{= total1} \hlopt{+} \hlstd{total2} \hlopt{-} \hlnum{2}\hlstd{)))} \hlopt{%>%}
    \hlkwd{select}\hlstd{(pval.single)}

  \hlstd{sd1.0} \hlkwb{<-} \hlkwd{which}\hlstd{(data}\hlopt{$}\hlstd{outcome.flag} \hlopt{==} \hlstr{"CONT"} \hlopt{&} \hlstd{data}\hlopt{$}\hlstd{sd1} \hlopt{==} \hlnum{0}\hlstd{)}
  \hlstd{sd2.0} \hlkwb{<-} \hlkwd{which}\hlstd{(data}\hlopt{$}\hlstd{outcome.flag} \hlopt{==} \hlstr{"CONT"} \hlopt{&} \hlstd{data}\hlopt{$}\hlstd{sd2} \hlopt{==} \hlnum{0}\hlstd{)}
  \hlstd{sd.0} \hlkwb{<-} \hlkwd{union}\hlstd{(sd1.0, sd2.0)}
  \hlstd{data[sd.0,} \hlstr{"pval.single"}\hlstd{]} \hlkwb{<-} \hlnum{NA} \hlcom{#Such that p-value here is not equal to zero}
  \hlcom{#----------------------------------------------------------------------------------------------#}

  \hlcom{#Outcome.flag == "IV"}
  \hlstd{data[IV.ind,} \hlstr{"pval.single"}\hlstd{]} \hlkwb{<-} \hlstd{data[IV.ind, ]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{pval.single} \hlstd{=} \hlnum{2}\hlopt{*}\hlstd{(}\hlnum{1}\hlopt{-}\hlkwd{pnorm}\hlstd{(}\hlkwd{abs}\hlstd{((effect)}\hlopt{/}\hlstd{se))))} \hlopt{%>%}
    \hlkwd{select}\hlstd{(pval.single)}
  \hlcom{#----------------------------------------------------------------------------------------------#}

  \hlcom{#Declare smd.ordl, cohensd and SMD of IV flag all as "smd.pool":}
  \hlstd{data} \hlkwb{<-} \hlstd{data} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{smd.pool} \hlstd{=} \hlkwd{case_when}\hlstd{(outcome.flag} \hlopt{==} \hlstr{"DICH"} \hlopt{~} \hlstd{smd.ordl,}
                                \hlstd{outcome.flag} \hlopt{==} \hlstr{"CONT"} \hlopt{~} \hlstd{cohensd,}
                                \hlstd{outcome.flag} \hlopt{==} \hlstr{"IV"} \hlopt{&}
                                  \hlstd{outcome.measure.merged} \hlopt{==} \hlstr{"SMD"} \hlopt{~} \hlstd{effect,}
                                \hlnum{TRUE} \hlopt{~} \hlnum{NA_real_}\hlstd{),}
           \hlkwc{se.smd.pool} \hlstd{=} \hlkwd{case_when}\hlstd{(outcome.flag} \hlopt{==} \hlstr{"DICH"} \hlopt{~} \hlkwd{sqrt}\hlstd{(var.smd.ordl),}
                                   \hlstd{outcome.flag} \hlopt{==} \hlstr{"CONT"} \hlopt{~} \hlkwd{sqrt}\hlstd{(var.cohensd),}
                                   \hlstd{outcome.flag} \hlopt{==} \hlstr{"IV"} \hlopt{&}
                                     \hlstd{outcome.measure.merged} \hlopt{==} \hlstr{"SMD"} \hlopt{~} \hlstd{se,}
                                   \hlnum{TRUE} \hlopt{~} \hlnum{NA_real_}\hlstd{))}
  \hlcom{#----------------------------------------------------------------------------------------------#}

  \hlcom{#Transform effect sizes:}
  \hlstd{smd.pool.ind} \hlkwb{<-} \hlkwd{which}\hlstd{(}\hlopt{!}\hlkwd{is.na}\hlstd{(data}\hlopt{$}\hlstd{smd.pool))}

  \hlcom{#Pearson correlation:}
  \hlstd{data[smd.pool.ind,} \hlstr{"cor.pearson"}\hlstd{]}\hlkwb{<-} \hlstd{data[smd.pool.ind, ]} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}
    \hlkwc{a} \hlstd{= ((total1} \hlopt{+} \hlstd{total2)}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{/}\hlstd{(total1}\hlopt{*}\hlstd{total2),}
    \hlkwc{cor.pearson}  \hlstd{= smd.pool}\hlopt{/}\hlkwd{sqrt}\hlstd{((smd.pool}\hlopt{^}\hlnum{2}\hlstd{)} \hlopt{+} \hlstd{a),}
    \hlkwc{var.cor.pearson} \hlstd{= ((a}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{*}\hlstd{se.smd.pool}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{/}\hlstd{(((smd.pool}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{+}\hlstd{a)}\hlopt{^}\hlnum{3}\hlstd{))} \hlopt{%>%}
    \hlkwd{select}\hlstd{(cor.pearson)}

  \hlstd{data[smd.pool.ind,} \hlstr{"var.cor.pearson"}\hlstd{]}\hlkwb{<-} \hlstd{data[smd.pool.ind, ]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}
    \hlkwc{a} \hlstd{= ((total1} \hlopt{+} \hlstd{total2)}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{/}\hlstd{(total1}\hlopt{*}\hlstd{total2),}
    \hlkwc{cor.pearson}  \hlstd{= smd.pool}\hlopt{/}\hlkwd{sqrt}\hlstd{((smd.pool}\hlopt{^}\hlnum{2}\hlstd{)} \hlopt{+} \hlstd{a),}
    \hlkwc{var.cor.pearson} \hlstd{= ((a}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{*}\hlstd{se.smd.pool}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{/}\hlstd{(((smd.pool}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{+}\hlstd{a)}\hlopt{^}\hlnum{3}\hlstd{))} \hlopt{%>%}
    \hlkwd{select}\hlstd{(var.cor.pearson)}

  \hlcom{#Fisher's z-score:}
  \hlstd{data[smd.pool.ind,} \hlstr{"z"}\hlstd{]} \hlkwb{<-} \hlstd{data[smd.pool.ind, ]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{z} \hlstd{=} \hlkwd{atan}\hlstd{(}\hlkwc{x} \hlstd{= cor.pearson),}
           \hlkwc{var.z}\hlstd{=} \hlnum{1}\hlopt{/}\hlstd{(total1} \hlopt{+} \hlstd{total2} \hlopt{-} \hlnum{3}\hlstd{))} \hlopt{%>%} \hlkwd{select}\hlstd{(z)}
  \hlstd{data[smd.pool.ind,} \hlstr{"var.z"}\hlstd{]} \hlkwb{<-} \hlstd{data[smd.pool.ind, ]} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{z} \hlstd{=} \hlkwd{atan}\hlstd{(}\hlkwc{x} \hlstd{= cor.pearson),}
           \hlkwc{var.z}\hlstd{=} \hlnum{1}\hlopt{/}\hlstd{(total1} \hlopt{+} \hlstd{total2} \hlopt{-} \hlnum{3}\hlstd{))} \hlopt{%>%} \hlkwd{select}\hlstd{(var.z)}

  \hlcom{#----------------------------------------------------------------------------------------------#}
  \hlstd{data}\hlopt{$}\hlstd{sig.single} \hlkwb{<-} \hlkwd{ifelse}\hlstd{(data}\hlopt{$}\hlstd{pval.single} \hlopt{<} \hlnum{0.05}\hlstd{,} \hlnum{1}\hlstd{,} \hlnum{0}\hlstd{)}
...
\hlcom{#Function to find sensitivity analyses.  The function gives all meta-analyses }
\hlcom{#that are subdivided}
\hlcom{#into sensitivity analyses a common number. }
\hlcom{#*to use with "data %>% mutate(.. = dupl.finder(..))"*}
\hlstd{dupl.finder} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{effects}\hlstd{,} \hlkwc{names}\hlstd{,} \hlkwc{metas}\hlstd{)\{}

  \hlstd{results} \hlkwb{<-} \hlkwd{rep}\hlstd{(}\hlnum{NA}\hlstd{,} \hlkwc{times} \hlstd{=} \hlkwd{length}\hlstd{(effects))}
  \hlstd{meta.double.marker} \hlkwb{<-} \hlnum{1}


  \hlkwa{for}\hlstd{(u} \hlkwa{in} \hlkwd{seq_along}\hlstd{(effects))\{}

    \hlkwa{if}\hlstd{(}\hlkwd{is.na}\hlstd{(results[u]))\{}

      \hlstd{double.indices} \hlkwb{<-} \hlkwd{which}\hlstd{(effects[u]} \hlopt{==} \hlstd{effects} \hlopt{&} \hlstd{names[u]} \hlopt{==} \hlstd{names)}

      \hlkwa{if}\hlstd{(}\hlkwd{length}\hlstd{(double.indices)} \hlopt{>} \hlnum{1}\hlstd{)\{}
    \hlcom{#If any meta-analysis has already duplicates, give all the same double marker}
        \hlkwa{if}\hlstd{(}\hlkwd{any}\hlstd{(}\hlopt{!}\hlkwd{is.na}\hlstd{(results[double.indices])))\{}

          \hlstd{already.marked} \hlkwb{<-} \hlkwd{which}\hlstd{(}\hlopt{!}\hlkwd{is.na}\hlstd{(results[double.indices]))}
          \hlstd{meta.double.marker.2} \hlkwb{<-} \hlkwd{unique}\hlstd{(results[double.indices[already.marked]])}
          \hlcom{#Collect meta.ids with same double markers}
          \hlstd{meta.ids.2} \hlkwb{<-} \hlkwd{unique}\hlstd{(metas[}\hlkwd{which}\hlstd{(results} \hlopt{%in%} \hlstd{meta.double.marker.2)])}
          \hlstd{meta.ids} \hlkwb{<-} \hlstd{metas[double.indices]} \hlcom{#Collect metas with the same results}

          \hlstd{meta.ids.pooled} \hlkwb{<-} \hlkwd{union}\hlstd{(meta.ids, meta.ids.2)}
          \hlstd{meta.indices} \hlkwb{<-} \hlkwd{which}\hlstd{(metas} \hlopt{%in%} \hlstd{meta.ids.pooled)}
          \hlstd{results[meta.indices]} \hlkwb{<-} \hlstd{meta.double.marker}

        \hlstd{\}} \hlkwa{else}\hlstd{\{}

          \hlstd{meta.ids} \hlkwb{<-} \hlstd{metas[double.indices]}
          \hlstd{meta.indices} \hlkwb{<-} \hlkwd{which}\hlstd{(metas} \hlopt{%in%} \hlstd{meta.ids)}
          \hlstd{results[meta.indices]} \hlkwb{<-} \hlstd{meta.double.marker}

        \hlstd{\}}
      \hlstd{\}} \hlkwa{else} \hlstd{results[u]} \hlkwb{<-} \hlnum{0}

      \hlstd{meta.double.marker} \hlkwb{<-} \hlstd{meta.double.marker} \hlopt{+} \hlnum{1}
    \hlstd{\}\}}
  \hlkwd{return}\hlstd{(results)}
\hlstd{\}}

\hlcom{#Function picks the largest meta-analysis within a set of sensitivity analyses,}
\hlcom{#and assigns it a "0" *to use with "data %>% mutate(.. = dupl.finder(..))":*}
\hlstd{dupl.max.finder} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{duplicate.index}\hlstd{,} \hlkwc{study.number}\hlstd{,} \hlkwc{metas}\hlstd{)\{}
  \hlstd{results} \hlkwb{<-} \hlkwd{rep}\hlstd{(}\hlnum{NA}\hlstd{,} \hlkwd{length}\hlstd{(duplicate.index))}

  \hlkwa{for}\hlstd{(u} \hlkwa{in} \hlkwd{seq_along}\hlstd{((duplicate.index)))\{}

    \hlstd{duplicate.indices} \hlkwb{<-} \hlkwd{which}\hlstd{(duplicate.index} \hlopt{%in%} \hlstd{duplicate.index[u])}

    \hlkwa{if}\hlstd{(duplicate.index[u]} \hlopt{!=} \hlnum{0}\hlstd{)\{}

      \hlstd{meta.ids} \hlkwb{<-} \hlstd{metas[duplicate.indices]}

      \hlstd{max.meta.id} \hlkwb{<-} \hlstd{meta.ids[}\hlkwd{which.max}\hlstd{(study.number[duplicate.indices])]}
      \hlstd{max.meta.indices} \hlkwb{<-} \hlkwd{which}\hlstd{(metas} \hlopt{%in%} \hlstd{max.meta.id)}
      \hlkwa{if}\hlstd{(}\hlkwd{length}\hlstd{(}\hlkwd{unique}\hlstd{(max.meta.id))} \hlopt{<} \hlnum{2}\hlstd{)\{}
        \hlstd{results[max.meta.indices]} \hlkwb{<-} \hlnum{0}
      \hlstd{\}} \hlkwa{else} \hlkwd{print}\hlstd{(}\hlstr{"error"}\hlstd{)}

    \hlstd{\}} \hlkwa{else} \hlstd{results[duplicate.indices]} \hlkwb{<-} \hlnum{0}

  \hlstd{\}}

  \hlstd{results[}\hlkwd{is.na}\hlstd{(results)]} \hlkwb{<-} \hlnum{1}
...
meta.info.bin <- data.ext2 %>% \hlkwd{filter}(outcome.flag == \hlstr{"DICH"}) %>%
  \hlkwd{group_by}(meta.id) %>%
  \hlkwd{mutate}(n.pre = \hlkwd{n}()) %>% \hlkwd{filter}(n.pre >= 10) %>%
  \hlkwd{filter}(!\hlkwd{all}(events1 == 0) & !\hlkwd{all}(events2 == 0)) %>% \hlcom{#No events}
...
meta.info.cont <- data.ext2 %>% \hlkwd{filter}(outcome.flag == \hlstr{"CONT"}) %>%
  \hlkwd{group_by}(meta.id) %>%
  \hlkwd{mutate}(n.pre = \hlkwd{n}()) %>% \hlkwd{filter}(n.pre >= 10) %>%
  \hlkwd{filter}(!\hlkwd{all}(mean1 == 0) & !\hlkwd{all}(mean2 == 0)) %>% \hlcom{#No means}
  \hlkwd{filter}(!\hlkwd{all}(sd1 == 0) | !\hlkwd{all}(sd2 == 0)) %>% \hlcom{#No sd's}
  \hlkwd{summarize}(dupl.remove = \hlkwd{unique}(dupl.remove),
            id = \hlkwd{unique}(id),
            outcome.desired = \hlkwd{unique}(outcome.desired),
            n.sig.single = \hlkwd{sum}(sig.single, na.rm = T),
...
meta.info.iv <- data.ext2 %>%
  \hlkwd{group_by}(meta.id) %>%
  \hlkwd{mutate}(n.pre = \hlkwd{n}()) %>% \hlkwd{filter}(n.pre >= 10) %>%
  \hlkwd{filter}(outcome.flag == \hlstr{"IV"}) %>%
  \hlkwd{filter}(outcome.measure.merged == \hlstr{"Rate Ratio"} | 
           outcome.measure.merged == \hlstr{"SMD"} |
           outcome.measure.merged == \hlstr{"MD"} | 
           outcome.measure.merged == \hlstr{"Hazard Ratio"} |
           outcome.measure.merged == \hlstr{"OR"} | 
           outcome.measure.merged == \hlstr{"RR"}) %>% #Remove outcomes w. unclear def.
  \hlkwd{summarize}(dupl.remove = \hlkwd{unique}(dupl.remove),
            id = \hlkwd{unique}(id),
            outcome.desired = \hlkwd{unique}(outcome.desired),
            n.sig.single = \hlkwd{sum}(sig.single, na.rm = T),
...
meta.info.pre <- \hlkwd{rbind}(meta.info.bin, meta.info.cont, meta.info.iv) 
\hlcom{#To save to control how many are excluded}

meta.info.pre <- meta.info.pre %>%
  \hlkwd{filter}(id %in% data.review$id[\hlkwd{which}(data.review$withdrawn == FALSE)]) 
\hlcom{#No withdrawn meta-analyses}


meta.info <- meta.info.pre %>% 
  \hlkwd{filter}(n.sig.single > 0) %>% \hlcom{#At least one significant result}
  \hlkwd{filter}((se.max^2)/(se.min^2) > 4) %>% \hlcom{#variance ratio > 4}
  \hlkwd{filter}(dupl.remove == 0) %>% \hlcom{#no duplicates}
  \hlkwd{filter}(outcome.desired == \hlstr{"efficacy"}) #Only efficacy outcomes
\hlcom{#--------------------------------------------------------------------------------------------------------------------#}

\hlcom{#Meta-analysis to get I2 (random effects meta-analysis):}
meta.id.vector.org.ranef <- meta.info$meta.id \hlcom{#Do meta-analysis for each of those}

meta.org.ranef.list <- \hlkwd{list}()
counter <- 0
\hlkwd{for}(u in meta.id.vector.org.ranef)\{
  counter <- counter + 1
  temp <- data.ext2 %>% \hlkwd{filter}(meta.id == u)
  \hlkwd{print}(\hlkwd{c}(u, counter))
  meta.org.ranef.list[[counter]] <- \hlkwd{meta.fct.ranef.mom}(temp)
\}

exclusion.estimates <- \hlkwd{cbind}(
  meta.id = meta.id.vector.org.ranef,
  n = \hlkwd{unlist}(\hlkwd{lapply}(meta.org.ranef.list, 
                    FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$k\})),
  Q = \hlkwd{unlist}(\hlkwd{lapply}(meta.org.ranef.list, 
                    FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$QE\})))


meta.info.pre2 <- \hlkwd{merge}(meta.info, exclusion.estimates, by = \hlstr{"meta.id"})
meta.info.I2 <- meta.info.pre2 %>% 
  \hlkwd{rowwise}() %>% \hlkwd{mutate}(I2  = \hlkwd{max}(0, (Q - n + 1)/Q)) \hlcom{#calculate I2}
meta.info <- meta.info.I2 %>% \hlkwd{filter}(n > 9) %>% 
  \hlkwd{filter}(I2 < 0.5) \hlcom{#exclude I2 >= 0.5}
meta.id.vector <- meta.info$meta.id \hlcom{#Vector of meta-ids that match the criteria.}

\hlkwd{save}(meta.id.vector, file =  \hlkwd{file.path}(PATH_RESULTS, \hlstr{"meta_id_vector.RData"}))
\hlkwd{save}(meta.info.I2, file =  \hlkwd{file.path}(PATH_RESULTS, \hlstr{"meta_id_I2.RData"}))
\hlcom{#--------------------------------------------------------------------------------------------------------------------#}

\hlcom{#Get more extensive information from the original dataset about the meta-analyses:}
meta.info.extended <- data.ext2 %>% 
  \hlkwd{filter}(meta.id %in% meta.id.vector) %>% 
  \hlkwd{group_by}(meta.id) %>%
...
\hlcom{	#Publication bias tests.}
	meta.tests.excess[[counter]] <- \hlkwd{tes.fct2}(data = data.ext2[data.ext2$meta.id == u,])
	meta.tests.harbord[[counter]] <- 
	  \hlkwd{metabias}(meta.analyses[[counter]], method.bias = \hlstr{"score"}, k.min = 2)
	meta.tests.peter[[counter]] <- 
	  \hlkwd{metabias}(meta.analyses[[counter]], method.bias = \hlstr{"peters"}, k.min = 2)
	meta.tests.schwarzer[[counter]] <- 
	  \hlkwd{metabias}(meta.analyses[[counter]], method.bias = \hlstr{"count"}, k.min = 2)
	meta.tests.rucker.mm[[counter]] <- 
	  \hlkwd{metabias}(meta.analyses.asd[[counter]], method.bias = \hlstr{"mm"}, k.min = 2)
	meta.tests.rucker.linreg[[counter]] <- 
	  \hlkwd{metabias}(meta.analyses.asd[[counter]], method.bias = \hlstr{"linreg"}, k.min = 2)
\}


\hlcom{# counter.unknown.bin.copas.error <- 0}
\hlcom{# for(u in meta.id.vector.bin)\{}
\hlcom{# 	if(length(meta.analyses.copas[[counter]]) < 3)\{}
\hlcom{# 		meta.analyses.copas[[counter]] <- c(NA, NA, NA)}
\hlcom{# 		counter.unknown.bin.copas.error <- counter.unknown.bin.copas.error + 1}
\hlcom{# 	\}}
\hlcom{# \}}
\hlcom{# print(c("unknown.bin.copas.errors = ", counter.unknown.bin.copas.error))}
\hlcom{#}
\hlcom{# bin.meta.list <- list(data.ext2, meta.id.vector.bin, meta.analyses,}
\hlcom{# 											meta.analyses.asd,}
\hlcom{# 											meta.analyses.reg,}
\hlcom{# 											meta.analyses.copas,}
\hlcom{# 											meta.tests.excess,}
\hlcom{# 											meta.tests.harbord,}
\hlcom{# 											meta.tests.peter,}
\hlcom{# 											meta.tests.rucker.mm,}
\hlcom{# 											meta.tests.schwarzer,}
\hlcom{# 											meta.tests.excess,}
\hlcom{# 											meta.tests.rucker.linreg)}
\hlcom{# save(bin.meta.list, file =  file.path(PATH_RESULTS, "meta_complete_list_bin.RData"))}
\hlcom{# #--------------------------------------------------------------------------------------------------------------------#}
\hlcom{# }
\hlcom{# #Load analysis data:}
\hlcom{# meta.id.vector.bin <- bin.meta.list[[2]]}
\hlcom{# meta.analyses <- bin.meta.list[[3]]}
\hlcom{# meta.analyses.reg <- bin.meta.list[[5]]}
\hlcom{# meta.analyses.copas <- bin.meta.list[[6]]}
\hlcom{# meta.analyses.trimfill <- bin.meta.list[[7]]}
\hlcom{# meta.tests.harbord <- bin.meta.list[[8]]}
\hlcom{# meta.tests.peter <- bin.meta.list[[9]]}
\hlcom{# meta.tests.rucker.mm <- bin.meta.list[[10]]}
\hlcom{# meta.tests.schwarzer <- bin.meta.list[[11]]}
\hlcom{# meta.tests.excess <- bin.meta.list[[12]]}
\hlcom{# meta.tests.rucker.linreg <- bin.meta.list[[13]]}


\hlcom{#Extract from lists:}
bin.results <- \hlkwd{data.frame}(
  meta.id = meta.id.vector.bin,
  meta.es.measure = \hlkwd{rep}(times = \hlkwd{length}(meta.id.vector.bin), \hlstr{"log risk ratio"}),
...
\hlcom{  #Meta-analysis:}
  ...
  Q = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$Q\})),
  pval.Q = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                         FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$pval.Q\})),
\hlcom{  #I2 = unlist(lapply(meta.analyses, FUN = function(meta.analysis)\{meta.analysis$I2.w\})),}
  tau = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                      FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$tau\})),
\hlcom{  #se.tau = unlist(lapply(meta.analyses, FUN = function(meta.analysis)\{meta.analysis$se.tau\})),}
  sparse = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                         FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$sparse\})),
...
\hlcom{#Extract from lists:}
cont.results <- \hlkwd{data.frame}(
  meta.id = meta.id.vector.cont,
  meta.es.measure = \hlkwd{rep}(\hlstr{"std. mean difference"}, times = \hlkwd{length}(meta.id.vector.cont)),
  
  n.sig.single2 = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, FUN = \hlkwd{function}(meta.analysis)\{\hlkwd{length}(\hlkwd{which}(meta.analysis$pval < 0.05))\})),
...
\hlcom{  #Meta-Analysis}
  ...
  tau = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                    FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$tau\})),
  k = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$k\})),
...
\hlcom{#--------------------------------------------------------------------------------------------------------------------#}
\hlcom{# IV OUTCOME META-ANALYSIS: -----------------------------------------------------------------------------------#}
\hlcom{#--------------------------------------------------------------------------------------------------------------------#}


\hlcom{# # -------- Uncomment to run analysis ----------}
meta.id.vector.iv <- 
  meta.info.extended$meta.id[\hlkwd{which}(meta.info.extended$outcome.flag == \hlstr{"IV"})]
meta.analyses <- \hlkwd{list}()
meta.analyses.reg <- \hlkwd{list}()
meta.analyses.copas <- \hlkwd{list}()
meta.analyses.trimfill <- \hlkwd{list}()
...
meta.tests.excess <- \hlkwd{list}()
counter <- 0
\hlkwd{for}(u in meta.id.vector.iv)\{
	counter <- counter + 1
	meta.analyses[[counter]] <- \hlkwd{metagen.iv}(data = data.ext2[data.ext2$meta.id == u,])
	\hlkwd{print}(\hlkwd{c}(u, counter))
...
meta.id.vector.noiv <- 
  meta.info.extended$meta.id[-\hlkwd{which}(meta.info.extended$outcome.flag == \hlstr{"IV"})]
meta.id.vector.smd <- 
  meta.info.extended$meta.id[\hlkwd{which}(meta.info.extended$outcome.measure.merged == \hlstr{"SMD"})]
meta.id.vector.z <- \hlkwd{union}(meta.id.vector.smd, meta.id.vector.noiv)
meta.id.vector.z.c <- meta.id.vector.z[-\hlkwd{c}(\hlkwd{which}(meta.id.vector.z == 104613),
                                          \hlkwd{which}(meta.id.vector.z == 104616),
                                          \hlkwd{which}(meta.id.vector.z == 153452),
                                          \hlkwd{which}(meta.id.vector.z == 163246),
                      \hlkwd{which}(meta.id.vector.z == 182958))] \hlcom{#Have zero total1..}
tmp.z <- data.ext2 %>% \hlkwd{group_by}(meta.id) %>% \hlkwd{mutate}(n = \hlkwd{n}()) %>% \hlkwd{filter}(n > 9) %>%
  \hlkwd{filter}(total1 + total2 > 3) %>% \hlkwd{filter}(total1 != 0 & total2 != 0)
meta.analyses <- \hlkwd{list}()
meta.analyses.reg <- \hlkwd{list}()
meta.analyses.copas <- \hlkwd{list}()
counter <- 0
\hlkwd{for}(u in meta.id.vector.z.c)\{
	counter <- counter + 1
	\hlkwd{print}(\hlkwd{c}(u,counter))
	meta.analyses[[counter]] <- \hlkwd{metacor}(cor = z, n = total1 + total2, 
	                   studlab = study.name, tmp.z[tmp.z$meta.id == u,])
	meta.analyses.reg[[counter]] <- \hlkwd{limitmeta}(meta.analyses[[counter]])
	meta.analyses.copas[[counter]] <- \hlkwd{auto.copas}(meta.analyses[[counter]], 
	                                             sig.level = 0.1)
\}
\hlcom{# zscore.meta.list <- list(tmp.z, meta.id.vector.z.c, meta.analyses, }
\hlcom{#meta.analyses.reg, meta.analyses.copas)}
\hlcom{# save(zscore.meta.list, file =  file.path(PATH_RESULTS, }
\hlcom{#"meta_complete_list_zscore.RData"))}
\hlcom{#--------------------------------------------------------------------------------------------------------------------#}
...
\hlcom{#List extraction:}
zscore.meta.analysis.estimates <- \hlkwd{cbind}(
  meta.id = meta.id.vector.z.c,
  est.z.fixef = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                    FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$TE.fixed\})),
  est.z.ranef = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                    FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$TE.random\})),
  se.est.z.fixef =  \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                    FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$seTE.fixed\})),
  se.est.z.ranef =  \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                    FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$seTE.random\})),
  
  est.z.reg = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses.reg, 
                    FUN = \hlkwd{function}(meta.adjust)\{meta.adjust$TE.adjust\})),
  se.est.z.reg = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses.reg, 
                    FUN = \hlkwd{function}(meta.adjust)\{meta.adjust$seTE.adjust\})),
  
  est.z.copas = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses.copas, 
                    FUN = \hlkwd{function}(meta.adjust)\{meta.adjust[1]\})),
  se.est.z.copas = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses.copas, 
                    FUN = \hlkwd{function}(meta.adjust)\{meta.adjust[2]\})))


\hlcom{#--------------------------------------------------------------------------------------------------------------------#}
\hlcom{# HEDGES G BASED META-ANALYSES: -------------------------------------------------------------------------------------#}
\hlcom{#--------------------------------------------------------------------------------------------------------------------#}

\hlcom{# # -------- Uncomment to run analysis ----------}
meta.id.vector.d <- meta.id.vector.z[-\hlkwd{c}(\hlkwd{which}(meta.id.vector.z == 25407))]
meta.analyses <- \hlkwd{list}()
meta.analyses.reg <- \hlkwd{list}()
meta.analyses.copas <- \hlkwd{list}()
counter <- 0
\hlkwd{for}(u in meta.id.vector.d)\{
  counter <- counter + 1
  \hlkwd{print}(\hlkwd{c}(u,counter))
  meta.analyses[[counter]] <- \hlkwd{metagen.bincont2}(data.ext2[data.ext2$meta.id == u,])
  meta.analyses.reg[[counter]] <- \hlkwd{limitmeta}(meta.analyses[[counter]])
  meta.analyses.copas[[counter]] <- \hlkwd{auto.copas}(meta.analyses[[counter]], 
                                               sig.level = 0.1)
\}
\hlcom{# cohensd.meta.list <- list(data.ext2, meta.id.vector.d, meta.analyses, }
\hlcom{#meta.analyses.reg, meta.analyses.copas)}
\hlcom{# save(cohensd.meta.list, file =  file.path(PATH_RESULTS, }
\hlcom{#"meta_complete_list_cohensd.RData"))}
\hlcom{#--------------------------------------------------------------------------------------------------------------------#}
...
\hlcom{#List extraction:}
cohensd.meta.analysis.estimates <- \hlkwd{cbind}(
  meta.id = meta.id.vector.d,
  est.d.fixef = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                  FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$TE.fixed\})),
  est.d.ranef = \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                  FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$TE.random\})),
  se.est.d.fixef =  \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                  FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$seTE.fixed\})),
  se.est.d.ranef =  \hlkwd{unlist}(\hlkwd{lapply}(meta.analyses, 
                  FUN = \hlkwd{function}(meta.analysis)\{meta.analysis$seTE.random\})),
...
meta.bin <- meta.f %>% \hlkwd{filter}(outcome.flag == \hlstr{"DICH"})
meta.cont <- meta.f %>% \hlkwd{filter}(outcome.flag == \hlstr{"CONT"})
meta.iv <- meta.f %>% \hlkwd{filter}(outcome.flag == \hlstr{"IV"})
\hlcom{#--------------------------------------------------------------------------------------------------------------------#}
...
\hlcom{# x <- foreach(i = 1:length(meta.id.vector.bin)) %dopar% }
\hlcom{#   # u <- meta.id.vector.bin[i]}
\hlcom{#   # counter <- counter + 1}
\hlcom{#   metabin(event.e = events1c, n.e = total1, event.c = events2c, n.c = total2,}
\hlcom{#           studlab = study.name, sm = "RR",}
\hlcom{#           method = "Inverse", data = data.ext2[data.ext2$meta.id == meta.id.vector.bin[i],])}
\hlcom{# }
\hlcom{# cl<-makeCluster(7)}
\hlcom{# registerDoParallel(cl)}
\hlcom{# }
\hlcom{# foreach(i = 1:length(meta.id.vector.bin)) %dopar%}
\hlcom{#   sqrt(i)}
