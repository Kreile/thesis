...
First the fixed effect meta-analysis will be explained. For convenience, notatotion will be identical whenever possible for both methods. Note that both methods can be used for continuous or dichotomous outcomes. For more details about the methods, see chapter 11 and 12 in \citep{metaanalysis}
...
\vspace{0mm}
The pooled estimate of the fixed effects model is then simply the weighted average with the weights given by the inverses of the variances, $w_{i}$.

\begin{align*}
\Delta &= \frac{\sum_{i = 1}^n w_{i}y_{i}}{\sum_{i = 1}^n w_{i}}
\end{align*}

and the variance is the reciprocal of the sum of the weights,

\begin{align*}
\nu &= \frac{1}{\sum_{i = 1}^n w_{i}}
\end{align*}

\section{Reporting Bias Adjustment}
One method to account for publication bias in meta-analysis is to apply the Trim and Fill adjustment method \citep{trimfill}. Its basic working principle is to mirror studies in a funnel plot that cause the funnel plot to be asymmetric and then to perform one of the meta-analyses as described in section {meta-analysis} with the mirrored studies. In the subsequent equations, the same terminology as for meta analyses will be used. 
