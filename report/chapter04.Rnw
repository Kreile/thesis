...
adjustment.p.log.hazard.ratio <- meta.f %>% filter(outcome.flag == "IV") %>% 
  ...
  xlab(expression(paste(italic(p),"-value of test statistic of Hedges"))) +
...
diff.d.fixef.reg <- meta.f %>% 
  ...
  theme_bw() + theme(legend.position = "none") + 
  scale_fill_manual(values  = c("seagreen1", "seagreen3", "green4")) + 
  xlab(expression(paste("Fixed effects - adjusted Hedges"))) + geom_vline(xintercept = 0, linetype = "dashed" )
#--------------------------------------------------------------------------------------------------------------------#
...
diff.d.ranef.reg <- meta.f  %>% 
  ...
  scale_fill_manual(values  = c("seagreen1", "seagreen3", "green4")) +  ylab("") +
xlab(expression(paste("Random effects - adjusted Hedges"))) + geom_vline(xintercept = 0, linetype = "dashed" )
#--------------------------------------------------------------------------------------------------------------------#
...
The Wald test statistics $p$-value for fixed and random effects meta-analyses and Copas and regression adjusted treatment effects have been calculated. They are shown in Figure \ref{fig:adjustment.stat} for meta-analysis based on $z$-score, std. mean difference and other effect measures (\texttt{outcome.flag} = \texttt{IV}). \\
...
%Figures \ref{fig:adjustment.stat.z} for the adjusted $z$-scores, \ref{fig:adjustment.stat.smd} for the adjusted std. mean difference and Cohen's d and \ref{fig:adjustment.stat.log.hazard.ratio} for the adjusted log hazard ratios.
