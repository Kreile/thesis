...
adjusted.evidence.fixed <- meta.f %>% filter(z.reg > -40) %>% 
  ...
  geom_text(data = increase_text, mapping = aes(x = -15, y = -4, label = label), 
            color = "chartreuse3") + 
  xlab("Adjusted z test statistic") + ylab("Fixed effects z test statistic")
...
adjusted.evidence.ranef <- meta.f %>% filter(z.reg > -40) %>% 
  ...
  geom_text(data = increase_text, mapping = aes(x = -15, y = -4, label = label), 
            color = "chartreuse3") + 
  xlab("Adjusted z test statistic") + ylab("Random effects z test statistic")
#--------------------------------------------------------------------------------------------------------------------#
...
publication.bias.fit <- lme(fixed = smd.pool ~ se.smd.pool, random = list(~ 1 | id, ~ 1 | meta.id),
...
print(xtable(publication.bias.fit.anova[,-1], caption = "Anova table for a generalized linear model for small study effects compared to the null model ('null.fit'). 'publication.bias.fit' denotes the model with random intercepts for meta-analyses and reviews.", 
...
<<echo =FALSE, results='asis', warning=FALSE>>=
publication.bias.rs.fit <- lme(fixed = smd.pool ~ se.smd.pool, 
                          random = list(~ 1 | id, ~ meta.id | meta.id),
                       weights = ~ se.smd.pool^2, method = "ML", data = reg.dat)
print(xtable(anova(publication.bias.fit, publication.bias.rs.fit)[,-1], caption = "Anova table for two generalized linear model fits. 'publication.bias.fit' denotes the model with random intercepts for meta-analyses and reviews, 'publication.bias.rs.fit' the model with additional random slopes per review.", 
...
publication.bias.fit <- lme(fixed = smd.pool ~ se.smd.pool, random = list(~ 1 | id, ~ 1 | meta.id),
                       weights = ~ se.smd.pool^2, method = "ML", data = reg.dat)
study.year.fit <- lme(fixed = smd.pool ~ se.smd.pool + mean.study.year, 
                random = list(~ 1 | id, ~ 1 | meta.id),
                weights = ~ se.smd.pool^2, method = "ML", data = reg.dat)
study.year.int.fit <- lme(fixed = smd.pool ~ se.smd.pool * mean.study.year, 
                    random = list(~ 1 | id, ~ 1 | meta.id),
                weights = ~ se.smd.pool^2, method = "ML", data = reg.dat)
