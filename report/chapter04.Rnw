...
p.samplesize.z  <- data.ext2 %>% 
  ...
  ylab(expression(paste("median absolute ",  italic(z), "-score"))) +
...
colnames(binary.tests.agreement) <- c("Excess significance, Schwarzer", "Excess significance, Peter",
                                      "Excess significance, R\\\"ucker", "Excess significance, Harbord",
                                      "Peter, Schwarzer", 'Schwarzer, R\\\"ucker', "Schwarzer, Harbord",
                                      "R\\\"ucker, Peter", "Harbord, Peter")
#--------------------------------------------------------------------------------------------------------------------#
...
labels <- c(pval1.harbord = "Harbord", pval1.peter = "Peter", pval1.rucker = "Rücker (Thompson)", pval1.rucker.linreg = "Rücker (Egger)", pval1.schwarzer = "Schwarzer", pval1.d.tes = "Excess significance")
...
p.dist.bin <- meta.bin %>% ungroup() %>% 
  ...
  theme(strip.text.x = element_text(size=9)) + ggtitle("Binary Outcomes") + 
...
p.dist.cont <- meta.cont %>% ungroup() %>% 
  ...
  theme(strip.text.x = element_text(size=9)) + ggtitle("Continuous Outcomes") + 
  theme(legend.position = "none")
...
colnames(cor.data) <- c("Rücker", "Harbord", "Peter", "Schwarzer", "Egger", "Thompson",
                        "Begg", "I-squared")
...
harbord.rucker.plot <- meta.f %>% 
  ...
  scale_color_manual(labels = c("systematic error", "95 % confidence intervals"), 
                     values = c("red", "blue")) +
  ggtitle(expression(paste("logit(", italic(p),"-value Harbord) - logit(", italic(p), "-value Rücker)" ))) + geom_smooth(method = "lm", col = "green")
#--------------------------------------------------------------------------------------------------------------------#
...
corrgram(cor.data, 
         ...
         col.regions = colorRampPalette(c("red", "red", "chartreuse", "royalblue", "navy")),
         cex.labels = 1.65, cex = .25, cex.cor = 2, pch = 1,
...
print(xtable(number.sig.tests, caption = "Number of significant test results per meta-analysis, separated
...
When leaving away the excess significance test, \Sexpr{mean.min.bin}\% of binary outcome tests and \Sexpr{mean.min.else}\% of \texttt{IV} and continuous outcome tests had at least one significant result. 
...
<<echo = FALSE, results = 'asis'>>=
print(xtable(test.agreement, align = "lcc", caption = "Overall proportion of agreement if publication bias is significant or non-significant, and if significant only. When comparing agreement if significant only, the proportion of the test with fewer significant results that is significant with another test as well is shown.",
             label = "test.agreement"), size = "scriptsize", sanitize.rownames.function = identity)
@
...
diff.z.fixef.reg <- meta.f %>% 
  ...
  xlab(expression(paste("Fixed effects", italic(z), "- adjusted ", italic(z)))) + geom_vline(xintercept = 0, linetype = "dashed" )
#--------------------------------------------------------------------------------------------------------------------#
...
diff.z.ranef.reg <- meta.f %>% 
  ...
  xlab(expression(paste("Random effects ", italic(z), " - adjusted ", italic(z)))) + geom_vline(xintercept = 0, linetype = "dashed" )
#--------------------------------------------------------------------------------------------------------------------#
...
diff.d.fixef.reg <- meta.f %>% 
  ...
  theme_bw() + theme(legend.position = "none") + 
  scale_fill_manual(values  = c("seagreen1", "seagreen3", "green4")) + 
  xlab(expression(paste("Fixed effects ", italic(d), " - adjusted ", italic(d)))) + geom_vline(xintercept = 0, linetype = "dashed" )
#--------------------------------------------------------------------------------------------------------------------#
...
diff.d.ranef.reg <- meta.f  %>% 
  ...
  scale_fill_manual(values  = c("seagreen1", "seagreen3", "green4")) +  ylab("") +
xlab(expression(paste("Random effects ", italic(d), " - adjusted ", italic(d)))) + geom_vline(xintercept = 0, linetype = "dashed" )
#--------------------------------------------------------------------------------------------------------------------#
...
rownames(adjustment.diff.table) <- c(paste("$z$:", c("Fixed - Copas", "Random - Copas", "Fixed - Regression", "Random - Regression")),
                                     paste("$d$:", c("Fixed - Copas", "Random - Copas", "Fixed - Regression", "Random - Regression")),
                                     paste("IV:", c("Fixed - Copas", "Random - Copas", "Fixed - Regression", "Random - Regression")))

colnames(adjustment.diff.table) <- c("5\\%", "25\\%", "50\\%", "75\\%", "95\\%", "mean", "= 0 (\\%)", "$\\geq$ 0 (\\%)", "> 0 (\\%)", "No adj. est. (\\%)")
...
colnames(missing.differences) <- c("id", "comparison.nr", "subgroup.nr",
                                   ...
                                   "$d$ fixed", "$d$ random", "$d$ Copas", "$d$ reg.")
...
adjusted.evidence.fixed <- meta.f %>% filter(z.reg > -40) %>% 
  ...
  geom_text(data = decrease_text, mapping = aes(x = -9, y = 6, label = label), 
            color = "tan3") + 
  geom_text(data = decrease_text, mapping = aes(x = 9, y = -6, label = label), 
            color = "tan3") + 
  ...
  ylab(expression(paste("Fixed effects z ", italic(z), " test statistic"), sep = ""))
...
c(5.1,6,4.1,2.1)
par(mfrow = c(2,3), cex.lab = 1.3, cex.axis = 1.5, las = 1, mar = c(5.1,6,4.1,2.1),mgp=c(4,1,0))
...
\begin{figure}
<<echo = FALSE, warning = FALSE, fig.height = 6.5>>=
...
The Copas selection model also allows to compute the number of missing (\ie unpublished) studies in a given meta-analysis, given that the model's assumptions are correct. It finds that \Sexpr{format(round(sum(meta.f$missing.copas, na.rm = T)), big.mark = ",")} are missing, which corresponds to \Sexpr{round((sum(meta.f$missing.copas, na.rm = T)/sum(meta.f$k))*100,1)}\% from all \Sexpr{format(round(sum(meta.f$k)), big.mark = ",")} analysed studies. Figure \ref{fig:copas.missing} shows a histogram of the overall fraction of missing studies. Note that random effects meta-analysis substitutes have been excluded (\Sexpr{sum(is.na(meta.f$missing.copas))} out of \Sexpr{format(dim(meta.f)[1], big.mark = ",")} to limit the size of the bin at no. missing studies = 0.
...
reg.dat <- reg.dat %>% group_by(meta.id) %>%
  mutate(tau.squared = metagen(TE = z.mirrored, seTE = se.z.mirrored)$tau^2,
         new.se.smd = sqrt(se.z.mirrored^2 + tau.squared))
...
publication.bias.fit <- lme(fixed = z.mirrored ~ se.z.mirrored, random = list(~ 1 | id, ~ 1 | meta.id),
                       weights = ~ se.z.mirrored^2, method = "ML", data = reg.dat)
# plot(y = reg.dat$z.mirrored, x = reg.dat$se.z.mirrored, cex = .1, xlab = "standard error", ylab = "z mirrored", las = 1)
# publication.bias.fit.coef <- summary(publication.bias.fit)$coefficients$fixed
# abline(a = publication.bias.fit.coef, col = "red", lty = 2)
...
null.fit <- lme(fixed = z.mirrored ~ 1, random = list(~ 1 | id, ~ 1 | meta.id),
                       weights = ~ se.z.mirrored^2, method = "ML", data = reg.dat)
publication.bias.fit.anova <- anova(null.fit, publication.bias.fit)
publication.bias.fit.anova[2,9] <- "<0.00001"
print(xtable(publication.bias.fit.anova[,-1], caption = "Anova table for a mixed linear model for small study effects compared to the null model ('null.fit'). 'publication.bias.fit' denotes the model with random intercepts for meta-analyses and reviews.",
             label = "anova.small.study", digits = c(1,1,1,1,1,1,1,1,5)),
      size = "scriptsize")
@

<<echo =FALSE, results='asis', warning=FALSE>>=
print(xtable(lmer.table(publication.bias.fit), align = "lccc",
             caption = "Coefficients and 95\\% confidence limits of the mixed linear model. 'se.z.mirrored' denotes the standard error of the $z$-score.", label = "coefficients.small.study"),  size = "scriptsize")
@
...
publication.bias.rs.fit <- lme(fixed = z.mirrored ~ se.z.mirrored,
                       ...
                       weights = ~ se.z.mirrored^2, method = "ML", data = reg.dat)
print(xtable(anova(publication.bias.fit, publication.bias.rs.fit)[,-1], caption = "Anova table for two mixed linear model fits. 'publication.bias.fit' denotes the model with random intercepts for meta-analyses and reviews, 'publication.bias.rs.fit' the model with additional random slopes per review.",
...
reg.dat <- reg.dat %>% filter(!is.na(study.year)) %>% ungroup() %>%
  ...
  group_by(meta.id) %>% mutate(mean.study.year = mean(study.year.center))
study.number.study.model <- dim(reg.dat)[1]
publication.bias.fit <- lme(fixed = z.mirrored ~ se.z.mirrored, random = list(~ 1 | id, ~ 1 | meta.id),
                       weights = ~ se.z.mirrored^2, method = "ML", data = reg.dat)
study.year.fit <- lme(fixed = z.mirrored ~ se.z.mirrored + mean.study.year,
                random = list(~ 1 | id, ~ 1 | meta.id),
                weights = ~ se.z.mirrored^2, method = "ML", data = reg.dat)
study.year.int.fit <- lme(fixed = z.mirrored ~ se.z.mirrored * mean.study.year,
                    random = list(~ 1 | id, ~ 1 | meta.id),
                weights = ~ se.z.mirrored^2, method = "ML", data = reg.dat)
print(xtable(anova(publication.bias.fit, study.year.fit, study.year.int.fit)[,-1], caption = "Anova table for three mixed linear model fits. 'Small study fit' denotes the model with the standard error as explanatory variable, 'study.year.fit' the model with standard error and the centered (- 2000) mean study year of the meta-analysis as explanatory variable, and 'study.year.int.fit' the model with interaction between the two.",
             label = "anova.lme.2", digits = c(1,1,1,1,1,1,1,1,6)), size = "scriptsize")
@
