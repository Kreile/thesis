...
adjusted.evidence.fixed <- meta.f %>% 
...
adjusted.evidence.ranef <- meta.f %>% 
  select(pval.se, z.fixef, z.ranef, z.reg, z.copas) %>% 
  gather(key = "method", value = "test.stat", z.reg:z.copas) %>%
...
However, there are some adjustments with large increase in evidence and clear dark orange color, as in the left plot in Figure \ref{fig:evidence.adjustment} (the three points close to x = 10). 
\begin{itemize}
