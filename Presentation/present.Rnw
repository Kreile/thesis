...
data.ext2 <- data.ext2 %>% mutate(study.year = ifelse(study.year < 2019, study.year, NA)) %>% 
    mutate(study.year = ifelse(study.year > 1920, study.year, NA))
biased.rev <- data.ext2 %>% filter(meta.id == 122943)
unbiased.rev <- data.ext2 %>% filter(meta.id == 181863)
meta.example <- meta.font(n.e = total1, mean.e = mean1, sd.e = sd1, n.c = total2, mean.c = mean2, sd.c = sd2, 
												 studlab = study.name, data = biased.rev)
meta.nexample <- meta.font(n.e = total1, mean.e = mean1, sd.e = sd1, n.c = total2, mean.c = mean2, sd.c = sd2, 
...
comparison.rev <- data %>% group_by(file.nr) %>% distinct(comparison.name) %>% count() %>% ungroup() %>% 
	summarise(lower = quantile(n, 0.05), median = quantile(n, 0.5), mean = mean(n), upper = quantile(n, 0.95))
meta.rev <- data.ext2 %>% group_by(file.nr) %>% distinct(meta.id) %>% count() %>% ungroup() %>% 
	summarise(lower = quantile(n, 0.05), median = quantile(n, 0.5), mean = mean(n), upper = quantile(n, 0.95))

#Global level
study.years <- data.ext2 %>% group_by(file.nr) %>% distinct(study.name, .keep_all = T) %>% ungroup() %>% 
...
sample.size <- data.ext2 %>% group_by(file.nr) %>% distinct(study.name, .keep_all = T) %>% ungroup() %>% 
...
dat_text <- data.frame(
  label = paste(c(sum(meta.cont$begg.test), sum(meta.cont$egger.test), sum(meta.cont$thomson.test)), "< 0.1,",
  							round(c(mean(meta.cont$begg.test), mean(meta.cont$egger.test), mean(meta.cont$thomson.test)),2)*100, "%"),
...
dat_text <- data.frame(
  label = paste(c(sum(meta.surv$begg.test), sum(meta.surv$egger.test), sum(meta.surv$thomson.test)), "< 0.1,",
  							round(c(mean(meta.surv$begg.test), mean(meta.surv$egger.test), mean(meta.surv$thomson.test)),2)*100, "%"),
  test.type   = c("pval.begg", "pval.egger", "pval.thomson"))
...
agree.surv <- meta.surv %>% mutate(n.sig = egger.test + thomson.test + begg.test) %>% 
  group_by(n.sig) %>% count %>% filter(n.sig > 0) %>% 
  ggplot(aes(y = nn, x = n.sig)) + theme_bw() + geom_col() + xlab("Number of significant tests") + ylab("count") + ggtitle("Continuous Outcomes")
...
# 	filter(dif > 2) %>% mutate(label = round(dif, 1), type = outcome.type == "bin") %>% arrange(label) %>% select(label, fixef.trimfill.s, meta.id, type)
...
copas.miss <- effect.diff %>% filter(!is.na(est.copas)) %>% group_by(fixef.copas) %>% mutate(dif = abs(est.fixef - est.copas)) %>% 
	filter(dif > 2) %>% mutate(label = round(dif, 1), type = outcome.type == "bin") %>% arrange(label) %>% select(label, fixef.copas.s, meta.id, type)
...
hist.copas <- effect.diff %>% filter(!is.na(est.copas)) %>% ggplot(aes(x = abs(est.fixef - est.copas), fill = factor(fixef.copas.s))) +
	...
	geom_text(data = copas.label, mapping = aes(label = label, x = 0, hjust = 0, y = 160), 
						color = "black") + scale_fill_discrete(name="Effect direction") + theme_bw() +
	theme(legend.position = "bottom", strip.text.x = element_text(size=15))
...
reg.ranef.miss <- effect.diff %>% group_by(fixef.reg) %>% mutate(dif = abs(est.fixef - est.reg.ranef)) %>% 
	filter(dif > 3) %>% mutate(label = round(dif, 1), type = outcome.type == "bin") %>% arrange(label) %>% select(label, fixef.reg.s, meta.id, type)
...
# 				filter(dif > 3) %>% mutate(label = round(dif, 1), outcome.type) %>% 
